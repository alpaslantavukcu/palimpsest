...
class PERCLOS():
    def __init__(self):
        ...
        self.frame_count =0
        load_detector = dlib.get_frontal_face_detector()
    ...
    def perclos_calculation(self):
        if self.Total_Blink !=0:
            self.PERCLOS = ((self.closed_eye_frame)/self.frame_count)*10
        else:
            self.PERCLOS = float(0)
"""
    ...
    def main(self):
        ...
        while True:
            ...
            if ret:
                ...
                self.frames +=1
                cv2.putText(frame, f"Prediction: {self.strongest_label}", (10,30), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                            (255, 0, 0), 2)
                cv2.putText(frame,"Point: {:.2f}".format(self.point),(10,50),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,0,255),2)
                cv2.putText(frame,"PERCLOS: {:.2f}".format(self.PERCLOS),(10,70),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,255,0),2)
